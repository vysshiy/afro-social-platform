...
export default function Step4_DateOfBirth({ value, onChange, onNext, onBack }: Props) {
  // Local string state so the user can type freely
  ...
  // Parse and bubble up whenever any field changes
  ...
  if (age >= 0 && age < 13)  { ageColor = '#ef4444'; ageText = 'Must be at least 13 years old'; }
  if (age > 120)              { ageColor = '#ef4444'; ageText = 'Please enter a valid year'; }
  ...
  function handleNext() {
    ...
    if (year < 1920 || year > new Date().getFullYear() - 13)
    ...
    >
      ...
      <div className="mb-6">
        ...
        <p className="text-sm mt-1" style={{ color: 'rgba(255,255,255,0.45)' }}>
          Type your birthday — Day / Month / Year
        </p>
      </div>
      ...
      <div className="flex gap-3 mb-5">
        ...
        />
        {/* Separator */}
        <div className="flex items-center pb-1 pt-7 text-white/25 text-2xl font-light select-none">/</div>
        <NumField
          id="dob-mm"
          label="Month"
          value={mm}
        ...
        <NumField
          ...
          value={yyyy}
          placeholder="YYYY"
          min={1920} max={new Date().getFullYear() - 13} maxLen={4}
          onChange={handleYyyy}
        />
      </div>

      {/* Hidden refs for focus chaining */}
      <input ref={mmRef}   className="sr-only" tabIndex={-1} readOnly />
      <input ref={yyyyRef} className="sr-only" tabIndex={-1} readOnly />
      ...
      <div className="flex items-start gap-2.5 px-1 mb-4">
        ...
        <p className="text-xs leading-relaxed" style={{ color: 'rgba(255,255,255,0.32)' }}>
          Example: Day <strong className="text-white/60">15</strong> · Month{' '}
          <strong className="text-white/60">08</strong> · Year{' '}
          <strong className="text-white/60">1995</strong> — the cursor moves
